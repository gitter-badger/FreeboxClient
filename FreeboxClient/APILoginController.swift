...
class APILoginController {
    ...
    init(delegate: APILoginControllerProtocol){
    ...
    func login() {
        let urlString = freeUrlString + "/api/v3/login/"
        get(urlString, id_cmd: 3)
    }
    
    func session(password: String) {
        let urlString = freeUrlString + "/api/v3/login/session/"
        var params = ["app_id":"freeboxclient","password":password]
        post(urlString, withParameters: params, id_cmd: 4)
    }
